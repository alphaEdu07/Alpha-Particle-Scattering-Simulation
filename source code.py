...
#k3 =Z_alpha * Z_Au / m_alpha * (C.c * 1e15)**2  # fm^2/(s^2 Mev)
...
'''
def solving_speed(val,b):   #Returns the speed at each point of motion 
    initial_value=np.array([-3e3,val,b,0])
    t = np.arange(0,1e-18,(1e-20))
    res = odeint(differential_equations,initial_value, t)
    v_x,v_y = res[:-1,1],res[:-1,3]#Speed in the x and y directions
    return v_x,v_y
'''
...
def draw(event):
    ...
    for i in range(0,6):# Cycle to different incidence initial conditions
        ...
        a[i]=b
        SUM=SUM+Energy
        #Energycompare=np.append(Energycompare,Energy)
    #maxenergy=np.max(Energycompare)
    # drawing
    for j in range(len(x_m)):#The number of rows of the matrix loops, and the loop order of the first row and then the column makes the image appear on the picture with different initial conditions of incidence at the same time
        plt.cla()# clear the chart
        plt.plot(0,0,'ko',ms=10) # Target particle position
        #plt.text(2,-2,'target particle',fontsize=20)# Mark the target particle position
        if max(a)<2000:
        ...
        plt.title('alpha particle scattering\n time: %d0zs'%(j))# Image caption
        plt.xlabel("x axis/fm")#The abscissa title
        plt.ylabel("y axis/fm")#The vertical axis title
        for k in range(0,6):#Loop through the number of columns of the matrix
            plt.plot(x_m[:j+1,k], y_m[:j+1,k], color=colorlist1[k],linewidth=1)# Draw a curve, and each time you draw from the first row of data to the j row, the trajectory of the particle is preserved
...
def random_draw(event):
    ...
    for j in range(len(x_m)):#The same principle as manually entering a drawing
        plt.subplot(1,2,1)
        plt.cla()# Clear the chart
        plt.plot(0,0,'ko',ms=10) # Target particle position
        ...
        for k in range(0,cycle_index):
            if abs(x_r[j,k])>3e3 or abs(y_r[j,k])>3e3:
                if k not in missing_list:
                    ...
                    if math.atan(tan_theta[0,k])<0  and  x_r[j,k]<0:#Second quadrant
                        theta_hist.append((math.atan(tan_theta[0,k])+np.pi)/np.pi*180)
                    elif math.atan(tan_theta[0,k])<0  and  x_r[j,k]>0:#Forth quadrant
                        theta_hist.append(abs(math.atan(tan_theta[0,k]))/np.pi*180)
                    elif math.atan(tan_theta[0,k])>0  and  x_r[j,k]>0:#First quadrant
                        theta_hist.append((math.atan(tan_theta[0,k]))/np.pi*180)
                    elif math.atan(tan_theta[0,k])>0  and  x_r[j,k]<0:#Third quadrant
